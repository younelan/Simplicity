...
class LinkDirectory extends \Opensitez\Simplicity\Component
{
	...
	function get_menus($app = [])
	{
		$menus = [
			"content" => [
				...
				"children" => [
					"linkdir" => ["component" => "linkdirectory", "page" => "default", "text" => "Link Directory", "category" => "all"],
				]
			],
	...
	{
		$this->explorer_model = new \Opensitez\Simplicity\Components\ExplorerModel($this->config_object);
	...
	function render_item($data)
	{
		...
		$body = $block_component->render_insert_text($item['body'] ?? "", $options, $app);
		$output .= $this->style;
		$output .= "
		<div id=node-details>
		<div class=field>
