...
class Form extends \Opensitez\Simplicity\Component
{
	private $vars = [];
	private $values = [];
	private $errors = [];
	...
	function get_menus($app = [])
	{
		...
		$menu = [
			"content" => [
				...
				"weight" => -2,
				"children" => [
					"select" => ["component" => "form", "page" => "default", "text" => "Forms", "category" => "all"],
				],
				"visible" => true,
			]
		];
	...
	function setvars($vars, $app = false)
	{
		...
		$this->framework->load_components(dirname(__DIR__) . "/widgets", 'Opensitez\\Simplicity\\Components', 'widgets');
	...
	///	$form_types = $this->framework->get_registered_type_list('widget');
	   ...
	   $field_component = $this->framework->get_registered_type('widget', strtolower($data_type));

	   $retval .= $field_component->render_field($data);
		// switch ($data_type) {
		// 	case "select":
